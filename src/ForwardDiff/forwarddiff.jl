...
_matrix_values(A::SparseMatrixCSC{Dual{T,V,N},IT}) where {T,V,N,IT} =
    SparseMatrixCSC(A.m, A.n, A.colptr, A.rowval, value.(A.nzval))
...
for solver in (:cg, :bicgstab)
    for matrix in (:(SparseMatrixCSC{V,IT} where {IT}), :(Matrix{V}))
        @eval begin
            function Krylov.$solver(
                A::$matrix,
                _b::Vector{Dual{T,V,N}};
                options...,
            ) where {T,V,N}
                b = value.(_b)
                m = length(b)
                ...
                for i = 1:m
                    px[i] = Partials{N,V}(Tuple(dxs[i, j] for j = 1:N))
                end
