...
function Krylov.cg(_A::SparseMatrixCSC{V, Int64}, _b::Vector{Dual{T, V, N}}; options...) where {T, V, N}
...
function Krylov.cg(A::Matrix{V}, _b::Vector{Dual{T, V, N}}; options...) where {T, V, N}
  b = value.(_b)
  m = length(b)
  dbs = Matrix{V}(undef, m, N)
  for i in 1:m
