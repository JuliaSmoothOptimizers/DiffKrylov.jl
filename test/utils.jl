...
function check_jacobian(solver, A, b)
  adJ = ForwardDiff.jacobian(x -> solver(A, x)[1], b)
  fdm = central_fdm(8, 1);
  fdJ = FiniteDifferences.jacobian(fdm, x -> solver(A, x)[1], copy(b))
  @test all(isapprox.(adJ, fdJ[1], atol=1e-4))
end
...
function check_derivatives_and_values_active_active(solver, A, b, x)
    ...
    isapprox(value.(dx), x)
    fd =fda[1] + fdb[1]
    @test isapprox(partials.(dx,1), fd, atol=1e-4)
end
...
function check_derivatives_and_values_active_passive(solver, A, b, x)
    ...
    fda = FiniteDifferences.jacobian(fdm, a -> A_one_one(a)[1], copy(A[1,1]))
    isapprox(value.(dx), x)
    @test isapprox(partials.(dx,1), fda[1], atol=1e-4)
end