...
function check_jacobian(solver, A, b)
    ...
    fdm = central_fdm(8, 1)
    fdJ = FiniteDifferences.jacobian(
        fdm,
        x -> solver(A, x; atol = atol, rtol = rtol)[1],
        copy(b),
    )
...
function test_enzyme_with(solver, A, b, M, N, ldiv = false)
    ...
    dA.dval[1, 1] = 1.0
    db.dval[1] = 1.0
    Enzyme.autodiff(Forward, driver!, dupsolver, dA, db, Const(M), Const(N), Const(ldiv))
    ...
    Enzyme.autodiff(Reverse, driver!, dupsolver, dA, db, Const(M), Const(N), Const(ldiv))
