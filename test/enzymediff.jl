...
@testset "Enzyme Rules" begin
    @testset "$MT" for MT in (Matrix, SparseMatrixCSC)
        @testset "($M, $N)" for (M, N) in ((I, I),)
            # Square unsymmetric solvers
            @testset "$solver" for solver in (Krylov.gmres, Krylov.bicgstab)
                A = []
                if MT == Matrix
            ...
            @testset "$solver" for solver in (Krylov.cg,)
                A, b = sparse_laplacian(4, FC = Float64)
                A = MT(A)
                test_enzyme_with(solver, A, b, M, N)
            end
